...
class ParticleFilter(Node):
    def __init__(self, node_name: str):
        ...
        self.init_pose_sub = self.create_subscription(PoseWithCovarianceStamped, "/initialpose", self.initposeCallback,
                                                      10)
    ...
    def initposeCallback(self, msg):
        ...
        for i in range(self.no_of_init_hypotheses):
            ...
            init_pose_orientation = tf_transformations.euler_from_quaternion(
    ...
    def prediction_step(self, current_pose, last_pose, last_used_pose):
        ...
        theta = tf_transformations.euler_from_quaternion(
            [last_used_pose.orientation.x, last_used_pose.orientation.y, last_used_pose.orientation.z,
             last_used_pose.orientation.w])[2]
        theta_final = tf_transformations.euler_from_quaternion(
            [last_pose.orientation.x, last_pose.orientation.y, last_pose.orientation.z, last_pose.orientation.w])[2]
    ...
    def closest_object_dist(self, x, y):
        ...
        for object in self.object_list:
            current_dist = np.sqrt(((object[0] - x) ** 2) + ((object[1] - y) ** 2)) + np.random.normal(0,
                                                                                                       0.01)  # Adding gaussian noise in sensor measurement
    ...
    def resample_around_top3(self, resampled_particles):
        ...
        for particle in unique_top_particles:
            for j in range(3):
                self.particles.append([np.random.normal(particle[0], 0.01), np.random.normal(particle[1], 0.01),
                                       np.random.normal(particle[2], 0.01), 1 / (len(unique_top_particles))])
    ...
    def timerCallback(self):
        ...
        distance_from_last_pose = np.sqrt(
            np.square(self.last_used_pose.position.x - self.last_pose.position.x) + np.square(
                self.last_used_pose.position.y - self.last_pose.position.y))
        theta_last_used_pose = tf_transformations.euler_from_quaternion(
            [self.last_used_pose.orientation.x, self.last_used_pose.orientation.y, self.last_used_pose.orientation.z,
             self.last_used_pose.orientation.w])[2]
        theta_last_pose = tf_transformations.euler_from_quaternion(
            [self.last_pose.orientation.x, self.last_pose.orientation.y, self.last_pose.orientation.z,
             self.last_pose.orientation.w])[2]
        ...
        if distance_from_last_pose < self.dist_thresh and abs(rotation_from_last_pose) < self.theta_thresh:
            ...
            ref_relative_position = self.cal_rel_position_of_laser_pts(self.last_scan,
                                                                       current_pose)  # Calculating how far measured laser points are from surrounding obstacles/occupied cells

            for i, particle in enumerate(self.particles):

                predicted_pose = self.prediction_step(particle, self.last_pose, self.last_used_pose)

                rel_position_for_each_particle = self.cal_rel_position_of_laser_pts(self.last_scan,
                                                                                    predicted_pose)  # Calculating how far measured laser points are from surrounding obstacles/occupied cells
